...
Setup (one-time):
    ...
    Save it next to this script as: firebase-key.json
...
# ---------------------------------------------------------------------------
ARDUINO_PORT = "/dev/ttyACM0"       # check with `ls /dev/tty*` after plugging in
ARDUINO_BAUD = 115200

GPS_PORT = "/dev/ttyUSB1"           # SIM7600G-H GNSS serial port (varies by wiring)
GPS_BAUD = 115200

FIREBASE_KEY_PATH = "firebase-key.json"   # must be a key generated FROM thermal-rover
FIREBASE_STORAGE_BUCKET = "thermal-rover.firebasestorage.app"

READINGS_COLLECTION = "readings"    # Firestore collection name the dashboard reads from
